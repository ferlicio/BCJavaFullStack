...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (token != null && token.startsWith("Bearer ")) { //token "valido"
            UsernamePasswordAuthenticationToken authToken = getAuthentication(token.substring(7)); //pega o token que começa a partir do index 7
            if (authToken != null) {
                SecurityContextHolder.getContext().setAuthentication(authToken);
                //guarda as informações do usuário no contexto do Spring
