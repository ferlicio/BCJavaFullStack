...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws
        ...
        //tenta autenticar o usuário
            try {
                //pega o usuário e a senha do request " {"username":"admin","password":"admin"} "
                User user = new ObjectMapper().readValue(request.getInputStream(), User.class);
                return authenticationManager.authenticate( //chama a autenticação do Spring Security
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain
         ...
         //gerar o token e devolver para o usuário que se autenticou
        AuthUserDetail user = (AuthUserDetail) authResult.getPrincipal();
        String token = jwtUtils.generateToken(user.getUsername());

        //define as permissões do usuário no header da resposta
